...
public class PhoneManagement {
	...
	public void NhapTT()
	{
		...
		for(int i=0;i<n;i++) {
			...
			Phones P = new Phones(tendt,hangdt,hedieuhanh,gia);
			list.add(P);
	}
}
	public void InTT() 
	...
	while (ePhone.hasMoreElements()) {
		...
		System.out.println("Ten Dien Thoai: "+phone.getTendt()+" Ten Hang: "+phone.getHangdt()+" He Dieu Hanh:, "+phone.getHedieuhanh()+" Gia:, "+phone.getGia());
	}
	}
	public void Timkiem()
	{
		Scanner key = new Scanner(System.in);
		System.out.println("Nhap  dien thoai can tim:");
		String tendt = key.nextLine();
		float gia = key.nextFloat();
		Enumeration ePhone = list.elements();
		System.out.println("\n--Thong tin tim kiem duoc--");
		while (ePhone.hasMoreElements())
		{
			Phones phone = (Phones)ePhone.nextElement();
				if(phone.getGia()==1000000) {
